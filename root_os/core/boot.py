    ...
    try:
        if system == "Linux" and "termux" not in os.getenv("PREFIX", "").lower():
            ...
            if not os.path.exists(boot_script):
                ...
                print("⚙️  [Auto-Start] Root הוגדרה לעלות אוטומטית באנדרואיד (דורש Termux:Boot).")
    except Exception as e:
...
def main():
    ...
    if not os.path.exists(root_db_path):
        ...
        choice = input("👉 תרצה שאבצע סריקת עומק (Monster Scan) כדי ללמוד את הארכיטקטורה שלו? (y/n): ")
        if choice.lower() == 'y':
            try:
