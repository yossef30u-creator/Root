    ...
    def _load_dynamic_ignore(self):
        ...
        if os.path.exists(rootignore_path):
            try:
                ...
                print(
                    f"⚠️ [Root Service] Failed to read .rootignore in {self.project_path}: {e}"
                )
    ...
    def trigger_ingestor(self, event, action_name):
        ...
        if current_time - last_trigger > COOLDOWN_SECONDS:
            ...
            try:
                ...
                print(
                    f"❌ [Root Service] שגיאה בהפעלת Ingestor עבור {project_name}: {e}"
                )
        ...
        while True:
            ...
            for proj in current_projects - watched_projects:
                if os.path.exists(proj):
                    ...
                    print(
                        f"👁️  [Root Service] מחובר כעת לפרויקט: {os.path.basename(proj)}"
                    )
            ...
            for proj in watched_projects - current_projects:
                ...
                print(
                    f"🚫 [Root Service] מנותק מהפרויקט (הוסר): {os.path.basename(proj)}"
                )
