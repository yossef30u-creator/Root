...
class ProjectScanner:
    def __init__(self, project_path):
        ...
        if not Config.API_KEY:
            print(
                "⚠️ [Scanner Warning] מפתח API חסר. הזיכרון הוקטורי עלול לא לעבוד כראוי."
            )
    ...
    def scan(self):
        ...
        for root, dirs, files in os.walk(self.project_path):
            dirs[:] = [
                ...
                and not self._is_user_ignored(os.path.join(root, d))
            ]
    ...
    def _finalize(self, file_list, duration):
        ...
        print(
            f"📊 סטטיסטיקה: {self.files_count} קבצים | {self.total_size_kb:.1f} KB | זמן: {duration:.2f} שניות."
        )
        print("-" * 50)
