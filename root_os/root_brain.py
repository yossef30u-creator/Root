...
class RootBrain:
    ...
    def execute_action(self, command):
        ...
        try:
            res = requests.post(
                f"{self.mcp_url}/execute", json={"command": command}, timeout=45
    ...
    def _is_error(self, output):
        ...
        return (
            "⚠️ failed" in lower_output
            or "❌ error" in lower_output
            or "not found" in lower_output
            or "syntax error" in lower_output
        )
    ...
    def solve_task(self, task, max_retries=3):
        ...
        try:
            ...
            while self._is_error(output) and attempts < max_retries:
                attempts += 1
                print(
                    f"🔄 [Brain] Error detected. Initiating self-correction (Attempt {attempts}/{max_retries})..."
                )

                command, output = self.self_correct(command, output)
                execution_log.append(
                    f"**Correction {attempts}:** `{command}`\n**Output:**\n```text\n{output}\n```"
                )
