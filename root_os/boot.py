...
def check_api_health():
    ...
    safe_key = (
        f"{api_key[:6]}...{api_key[-4:]}" if len(api_key) > 10 else "Invalid Length"
    )
...
is_mobile = (
    ...
    or "android" in platform.processor().lower()
)
...
async def get_status():
    ...
    status_data = {
        ...
        "memory_type": (
            "Vector DB (Chroma)" if memory and memory.use_chroma else "Lite DB (JSON)"
        ),
...
async def execute_command(request: CommandRequest):
    ...
    for bad_cmd in dangerous_commands:
        if bad_cmd in cmd:
            return {
                "command": cmd,
                "output": "❌ Security Alert: Command blocked by Root OS.",
            }
    ...
    try:
        ...
        return {
            "command": cmd,
            "output": "⚠️ Failed: Command timed out after 45 seconds.",
            "success": False,
        }
    except Exception as e:
        return {
