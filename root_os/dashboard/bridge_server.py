        ...
        atomic_write_json(
            STATE_FILE,
            {
                ...
                "history": [
                    {
                        "timestamp": time.time(),
                        "message": f"Root OS Engine Initialized in {PROJECT_DIR}.",
                    }
                ],
                "user_credits": 1000,
...
def chat():
    ...
    try:
        ...
        if context_items:
            context_text = "\n".join(
                [
                    f"Context from {item[1]['metadata'].get('path', 'memory')}:\n{item[1]['text']}"
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": f"You are Root OS, an expert architect. Use this project context if relevant:\n{context_text}",
                },
    ...
    if os.path.exists(root_md_path):
        try:
            ...
            if len(nodes) == 1:
                ...
                links.append(
                    {"source": "Core Engine", "target": "Waiting for Architecture"}
                )
