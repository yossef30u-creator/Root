...
class GitHubManager:
    ...
    def __init__(self):
        ...
        if not self.token or not self.repo:
            print(
                "⚠️ [GitHubManager] Missing credentials (GITHUB_TOKEN or GITHUB_REPOSITORY). Operating in OFFLINE mode."
            )
    ...
    def fetch_tasks(self, label="root-task", state="open"):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ [GitHubManager] Failed to fetch tasks: {response.status_code} - {response.text}"
                )
                return []
    ...
    def comment_on_issue(self, issue_number, comment_body):
        ...
        if not self.is_active:
            print(
                f"📴 [Offline] Would have commented on #{issue_number}: {comment_body[:30]}..."
            )
            return False
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(
                    f"❌ [GitHubManager] Failed to post comment on #{issue_number}: {response.status_code}"
                )
                return False
        except requests.exceptions.RequestException as e:
    ...
    def close_issue(self, issue_number):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ [GitHubManager] Failed to close issue #{issue_number}: {response.status_code}"
                )
                return False
        except requests.exceptions.RequestException as e:
    ...
    def create_issue(self, title, body, labels=None):
        ...
        try:
            data = {"title": title, "body": body, "labels": labels or ["root-task"]}
            response = requests.post(
            ...
            if response.status_code == 201:
                ...
                print(
                    f"❌ [GitHubManager] Failed to create issue: {response.status_code}"
                )
                return None
        except requests.exceptions.RequestException as e:
