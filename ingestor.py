...
def update_manifest(analysis, critic_feedback=None):
    """מעדכן או יוצר מחדש את ROOT.md בעיצוב דשבורד מקצועי"""
    ...
    # 🔴 כאן הקסם: אם הקובץ לא קיים (כי מחקת אותו), המערכת תבנה אותו מאפס!
    if not os.path.exists(Config.MANIFEST_PATH):
        print(f"[Root] Recreating missing {Config.MANIFEST_PATH}...")
        ...
        print("✅ [Root] ROOT.md created fresh.")
    else:
        # אם הוא כבר קיים - מעדכנים רק את שורות הסטטוס בפורמט החדש
        with open(Config.MANIFEST_PATH, "r", encoding="utf-8") as f:
            lines = f.readlines()
        
        with open(Config.MANIFEST_PATH, "w", encoding="utf-8") as f:
            skip_old_status = False
            for line in lines:
                if line.startswith("> **Status:**"):
                    f.writelines(content_blocks)
                    skip_old_status = True
                elif skip_old_status and line.startswith(">"):
                    continue
                else:
                    skip_old_status = False
                    f.write(line)
...
# =====
# Triggering fresh ROOT.md generation
